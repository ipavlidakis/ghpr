...
struct MarkdownBlockTests {
    ...
    func htmlTable() {
        ...
        #expect(MarkdownBlock.parse(text) == [
            .table(header: ["", "1 Warning"], rows: [["⚠️", "Big PR"]]),
            .rightAlignedParagraph("Generated by 🚫 Danger")
        ])
    }
    ...
    @Test("tables without leading pipes parse")
    func tableWithoutLeadingPipes() {
        let text = """
        Test name | Commit | Details | Required | Rerun command
        --- | --- | --- | --- | ---
        pull-kubernetes-dra-integration | 530be8d | [link](https://example.com) | false | `/test pull-kubernetes-dra-integration`
        """

        #expect(MarkdownBlock.parse(text) == [
            .table(
                header: ["Test name", "Commit", "Details", "Required", "Rerun command"],
                rows: [[
                    "pull-kubernetes-dra-integration",
                    "530be8d",
    ...
    @Test("table cells decode inline HTML breaks")
    func tableCellsDecodeBreaks() {
        let text = """
        Layer / File(s) | Summary
        --- | ---
        Safety <br> src/install/bin.rs | Checks &amp; validates paths
        """

        #expect(MarkdownBlock.parse(text) == [
            .table(
                header: ["Layer / File(s)", "Summary"],
                rows: [["Safety \n src/install/bin.rs", "Checks & validates paths"]]
            )
        ])
    }
    ...
    @Test("GitHub alerts unwrap blockquote markdown and details")
    func githubAlert() {
        let text = """
        > [!IMPORTANT]
        >
        ...
        >
        > - [ ] <!-- {"checkboxId": "abc"} --> 🔍 Trigger review
        """

        let blocks = MarkdownBlock.parse(text)

        guard blocks.count == 1, case .alert(let kind, let inner) = blocks[0] else {
            Issue.record("expected a GitHub alert block, got \(blocks)")
            return
        }
        #expect(kind == "important")
        #expect(inner.contains(.heading(level: 2, text: "Review skipped")))
        #expect(inner.contains(.paragraph("Auto reviews are disabled on base/target branches other than the default branch.")))
        #expect(inner.contains(.task(checked: false, text: "🔍 Trigger review")))

        guard case .details(let summary, let detailBlocks) = inner.first(where: {
            if case .details = $0 { true } else { false }
        }) ?? .rule else {
            Issue.record("expected details inside alert, got \(inner)")
            return
        }
        #expect(summary == "⚙️ Run configuration")
        #expect(detailBlocks.contains(.paragraph("**Configuration used:** defaults")))
    }
    ...
    func htmlComments() {
        ...
        // Comment lines disappear; surrounding text keeps GitHub-style line
        // separation as separate paragraphs.
        #expect(MarkdownBlock.parse(text) == [
            .paragraph("before"),
            .paragraph("after"),
        ])
