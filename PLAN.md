...
- **Distribution is a GitHub release + Homebrew tap (milestone 8), not `make install`.** The binary is not standalone — the tree-sitter grammar bundles must live next to the real executable — so `make dist` packages binary + bundles into a tarball, the release workflow uploads it on version tags, and `ipavlidakis/homebrew-tap`'s formula installs everything into `libexec` with a `bin` symlink (symlinks are fine: the bundle lookup resolves the real executable path). Split-view diff layout remains the one deferred milestone-8 item.
- **The CLI main must stay synchronous (milestone 4, debugged the hard way).** With an `AsyncParsableCommand` root, Swift's async main parks the main thread in the concurrency drain loop; once `NSApplication.run()` starts inside one of those jobs, every later MainActor continuation starves — `await`s never resume in the UI even though AppKit events keep flowing (the syntax-highlighting actor finished its work but could never deliver results back). The root command is therefore a synchronous `ParsableCommand`; UI commands call `MainActor.assumeIsolated` + `AppBootstrap.run`, and non-UI subcommands bridge async work through `AsyncBridge` (a main-run-loop pump). Do not convert the root command back to async.
- **Highlighting without Neon (milestone 4):** SwiftTreeSitter is used directly — Neon's incremental machinery (edits, invalidation) only pays off for editable documents, and diff content is immutable per file. A `SyntaxHighlighter` actor parses each hunk side off the main thread, caches per file path, and maps highlight captures back onto diff lines. Grammar version pins matter: several upstream tags (JS 0.25.0, Python 0.25.0, YAML 0.7.1+, ObjC 3.x) ship manifests whose relative-path `fileExists` check silently drops `scanner.c` and breaks linking. SwiftTreeSitter's own bundle lookup also assumes Xcode bundle layout, so `SupportedLanguage` probes both flat (`swift build`) and `Contents/Resources` layouts itself.
- **Row host decision (milestone 4 spike, measured):** a manual `NSTableView` wrapper (`DiffTableView`) with `usesAutomaticRowHeights = false` and fixed 18 pt rows. Both SwiftUI attempts failed on large files: `LazyVStack` in a `ScrollView` dropped frames (no row recycling), and `List` produced multi-hundred-millisecond main-thread hitches even in release — an Instruments trace showed AttributeGraph page allocation and KVO/notification observer teardown storms from hosting thousands of rows on file open/switch. With fixed row heights the table does zero offscreen work. `FileDiffView` owns its own scrolling — parents must not wrap it in a `ScrollView`. Cells render gutter+marker+code as one `NSAttributedString` per row; intra-line emphasis is paired eagerly but word-diffed on demand per visible row. Inline annotation slots (review threads) will be dedicated variable-height table rows in milestone 5 (the SwiftUI view-builder slot went away with `List`). The unbundled `NSApplication`-from-CLI window works as designed — no `.app` wrapper needed.
